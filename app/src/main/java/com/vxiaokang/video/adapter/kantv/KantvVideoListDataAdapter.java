package com.vxiaokang.video.adapter.kantv;
 ...
 * 列表
 */
public class KantvVideoListDataAdapter extends RecyclerView.Adapter<KantvVideoListDataAdapter.RecyclerViewHolder> {
    ...
    public KantvVideoListDataAdapter(Activity activity, List<VideoInfoBean> list) {
     //   Log.d("TAG--",list.size()+"");
        mActivity = activity;
        mList = list;
    }
    ...
    public RecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
       // Log.d("TAG--","onCreateViewHolder");
        ...
        View view = LayoutInflater.from(mActivity).inflate(R.layout.yinshi_video_item, null);
    ...
    public void onBindViewHolder(final RecyclerViewHolder holder, int position) {
        try{
            ...
            String views  = mList.get(position).getKeywords();
            if(StringUtils.isEmpty(views)){
                views = "";
            }
            holder.viewsView.setText(views);
            holder.class_item_times.setText(""+mList.get(position).getAuthor());
        }catch (Exception ex){
            ex.printStackTrace();
        }
    }
