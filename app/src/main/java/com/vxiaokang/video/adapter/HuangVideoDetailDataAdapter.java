...
import com.vxiaokang.video.util.codec.StringUtils;
 ...
 * 列表
 */
public class HuangVideoDetailDataAdapter extends RecyclerView.Adapter<HuangVideoDetailDataAdapter.RecyclerViewHolder> {
    ...
    public HuangVideoDetailDataAdapter(Activity activity, List<VideoInfoBean> list) {
     //   Log.d("TAG--",list.size()+"");
        mActivity = activity;
        mList = list;
    }
    ...
    public RecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
       // Log.d("TAG--","onCreateViewHolder");
        ...
        View view = LayoutInflater.from(mActivity).inflate(R.layout.huang_video_detail_item, null);
      ...
      //  Log.d("TAG---class", String.valueOf(position));
        try{
           ...
           // Glide.with(mActivity).load(R.mipmap.default_img).into(holder.imageView);
            ...
            if(StringUtils.equals(views,"0")){
                holder.viewsView.setVisibility(View.GONE);
            }else{
                holder.viewsView.setText("播放："+views);
            }
            holder.class_item_times.setText("时长："+mList.get(position).getAuthor());
