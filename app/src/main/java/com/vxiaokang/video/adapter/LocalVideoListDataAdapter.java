 ...
 * 列表
 */
public class LocalVideoListDataAdapter extends RecyclerView.Adapter<LocalVideoListDataAdapter.RecyclerViewHolder> {

    private List<ResultVideoItemInfoBean> mList;//数据源
    private Activity mActivity;
    private int currentPosition = 0;
    private OnItemClickListener onItemClickListener;//声明一下这个接口

    public LocalVideoListDataAdapter(Activity activity, List<ResultVideoItemInfoBean> list) {
     //   Log.d("TAG--",list.size()+"");
        mActivity = activity;
        mList = list;
    }
    ...
    public RecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
       // Log.d("TAG--","onCreateViewHolder");
        ...
        View view = LayoutInflater.from(mActivity).inflate(R.layout.local_huang_video_item, null);
    ...
    public void onBindViewHolder(final RecyclerViewHolder holder, int position) {
        try{
            if(TextUtils.isEmpty(mList.get(position).getImg3())){
                if(null != mActivity) {
               ...
               // Glide.with(mActivity).load(R.mipmap.default_img).into(holder.imageView);
                if(null != mActivity) {
                    Glide.with(mActivity).load(mList.get(position).getImg3())
           ...
           // holder.viewsView.setText("播放："+views);
            holder.class_item_times.setText(""+mList.get(position).getPlays());
        }catch (Exception ex){
            ex.printStackTrace();
        }
    }
    ...
    public void updateData(List<ResultVideoItemInfoBean> list) {
        mList.clear();
        mList.addAll(list);
        notifyDataSetChanged();
    }

    public void addData(List<ResultVideoItemInfoBean> list) {
       // mList.clear();
        mList.addAll(list);
        notifyDataSetChanged();
    }
