 ...
 * 首页
 */
public class HuangIndexVideoAdapter extends RecyclerView.Adapter<HuangIndexVideoAdapter.RecyclerViewHolder> {

    private List<IndexVideoBean> mList;//数据源
    private Activity mActivity;
    private OnItemClickListener onItemClickListener;//声明一下这个接口
    private int currentPosition = 0;
    private HuangIndexVideoChildAdapter mItemRecyclerViewAdapter;


    public HuangIndexVideoAdapter(Activity activity, List<IndexVideoBean> list) {
       // Log.d("TAG--",list.size()+"");
        mActivity = activity;
        mList = list;
    }
    ...
    public RecyclerViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View view = LayoutInflater.from(mActivity).inflate(R.layout.huang_index_video_item, null);
    ...
    public void onBindViewHolder(final RecyclerViewHolder holder, int position) {
        ...
        try{
            ...
            holder.index_video_rec_view.setLayoutManager(new GridLayoutManager(mActivity, 2,
                    GridLayoutManager.VERTICAL, false));
            mItemRecyclerViewAdapter = new HuangIndexVideoChildAdapter(mActivity,mList.get(position).getVideoList(),position);
