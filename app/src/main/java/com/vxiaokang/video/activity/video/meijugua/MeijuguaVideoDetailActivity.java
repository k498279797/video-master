 ...
 * 详情页面
 */
public class MeijuguaVideoDetailActivity extends AppCompatActivity implements View.OnClickListener{
    private String TAG = "MeijuguaVideoDetailActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_meijugua_video_detail);
    ...
    private void initView() {
      ...
      //  String  title = getIntent().getStringExtra("title");
       ...
       // GridLayoutManager gridLayoutManager2 = new GridLayoutManager(this, 5);
        ...
        tabAdapter = new MeijuguaVideoDetailTabAdapter(this,tabList,0);
        ...
        classAdapter.setOnItemClickListener(position -> {
            try{
                ...
                Intent intent = new Intent(this, MeijuguaVideoDetailPlayActivity.class);
        ...
        try{
            new Thread(() -> {
                ...
                try{
                    ...
                    String img = document.select("div.detail-pic img").first().attr("src");
                    String name = document.select("div.detail-title h1").first().text();
                    String desc = document.select("div.articlecon").first().text();
                    Elements dlall = document.select("div.info dd");
                    for(Element itElement : dlall) {
                        tagsTemp.add(itElement.text());
                        //System.out.println(itElement.text());
                    }
                    if(!com.blankj.utilcode.util.StringUtils.isEmpty(img)){
                        if(img.startsWith("http")){
                            itemTemp.setCoverImage(img);
                        }else{
                            itemTemp.setCoverImage(MeijuguaConstant.getReqDomain()+img);
                        }
                    }
                  ...
                  //  System.out.println(img);
                   // Element panelHead = document.select(".myui-panel__head").first();
                    Elements tabs = document.select("div.sidetitle h3");
                    for(Element itElement : tabs) {
                      // System.out.print(itElement.text()+"  ");
                        tabListTemp.add(itElement.text());
                    }

                    Elements elements = document.select("div.sidecon");
