package com.vxiaokang.video.activity.video.gaytube;
...
import com.vxiaokang.video.activity.video.gaytube.bean.GaytubeConstant;
 ...
 * 列表
 */
public class GaytubeVideoListActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_dude_video_list);
    ...
    private void initSpinnerRegion(){
        String[] tabName = GaytubeConstant.regionName;
        //声明一个下拉列表的数组适配器
        ...
        regionSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchRegion = GaytubeConstant.regionId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerType(){
        String[] tabName = GaytubeConstant.typeName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        typeSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchType = GaytubeConstant.typeId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerLanguage(){
        String[] tabName = GaytubeConstant.languageName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        languageSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchLanguage = GaytubeConstant.languageId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerTab(){
        String[] tabName = GaytubeConstant.tabName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        tabSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                categoryId = GaytubeConstant.tabId[position]+"";
                pageIndex = 1;
                if(initFlag){
                    setDataList();
                }
    ...
    private void initSpinnerYear(){
        String[] tabName = GaytubeConstant.yearName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        yearSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
               String  routLine = GaytubeConstant.yearId[position]+"";
               if(!StringUtils.isEmpty(routLine)){
                   GaytubeConstant.setReqDomain(routLine);
               }
                if(initFlag){
                    setDataList();
                }
    ...
    private void initSpinnerOrder(){
        String[] tabName = GaytubeConstant.orderName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        orderSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                orderBy = GaytubeConstant.orderId[position]+"";
                if(initFlag){
                    pageIndex = 1;
                    searchword = "";
                    setDataList();
    ...
    private void initView() {
        ...
        if(!TextUtils.isEmpty(tabName)){
            ...
            for(int i = 0; i < GaytubeConstant.tabName.length ; i++){
                if(StringUtils.equals(tabName, GaytubeConstant.tabName[i])){
        ...
        classAdapter.setOnItemClickListener(position -> {
            try{
                ...
                Intent intent = new Intent(this, GaytubeVideoDetailPlayActivity.class);
    ...
    private void setDataList() {
        try{
            ...
            new Thread(() -> {
                ...
                //https://gaydude.me/popular/2/
                String apiUrl = "";
                if(pageIndex > 1){
                    // https://gaydude.me/popular/2/
                    apiUrl = GaytubeConstant.getReqDomain() +"%s"+"%s/";
                    apiUrl = String.format(apiUrl,categoryId,pageIndex);
                }else{
                    apiUrl = GaytubeConstant.getReqDomain() +"%s";
               ...
               Log.d(TAG,"url = "+apiUrl);
                try{
                    ...
                    if(null == document){
                        ...
                        Elements elements = document.select(".thumbs .thumb");
                        for(Element element: elements) {
                           // Log.d(TAG,"element = "+element);
                            ...
                            String name = video.attr("title");

                            Element imgElement = element.select(".thumb-container img").first();
                            String img = imgElement.attr("src");
                            if(StringUtils.isEmpty(img)){
                                img = imgElement.attr("data-src");
                            }

                            String duration = element.select("span.thumb-duration").first().text();
                            String views = element.select("span.thumb-views").first().text();
                           ...
                           /* Log.d(TAG,"link = "+link);
                            ...
                            info.setTitle(name);
                            info.setLink(GaytubeConstant.getReqDomain()+link);
                            info.setAuthor(views);
                            info.setKeywords(duration);
                            if(!StringUtils.isEmpty(img)){
                                if(img.startsWith("http")){
                                    info.setCoverImage(img);
                                }else{
                                    info.setCoverImage(GaytubeConstant.getReqDomain()+img);
                                }
                            }
