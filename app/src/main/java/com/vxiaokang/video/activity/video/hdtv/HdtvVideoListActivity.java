 ...
 * 列表
 */
public class HdtvVideoListActivity extends AppCompatActivity implements View.OnClickListener{
    private String TAG = "HdtvVideoListActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hdtv_video_list);
    ...
    private void initSpinnerRegion(){
        String[] tabName = HdtvConstant.regionName;
        //声明一个下拉列表的数组适配器
        ...
        regionSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchRegion = HdtvConstant.regionId[position]+"";
                if(initFlag){
                    pageIndex = 1;
                    setDataList();
                }
    ...
    private void initSpinnerType(){
        String[] tabName = HdtvConstant.typeName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        typeSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchType = HdtvConstant.typeId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerLanguage(){
        String[] tabName = HdtvConstant.languageName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        languageSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchLanguage = HdtvConstant.languageId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerTab(){
        String[] tabName = HdtvConstant.tabName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        tabSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                categoryId = HdtvConstant.tabId[position]+"";
                if(initFlag){
                    pageIndex = 1;
                    setDataList();
                }
    ...
    private void initSpinnerYear(){
        String[] tabName = HdtvConstant.yearName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        yearSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchYear = HdtvConstant.yearId[position]+"";
                if(initFlag){
                    pageIndex = 1;
                    setDataList();
                }
    ...
    private void initSpinnerOrder(){
        String[] tabName = HdtvConstant.orderName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        orderSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                orderBy = HdtvConstant.orderId[position]+"";
                if(initFlag){
                    pageIndex = 1;
                    setDataList();
                }
    ...
    private void initView() {
        ...
        if(!TextUtils.isEmpty(tabName)){
            ...
            for(int i = 0; i < HdtvConstant.tabName.length ; i++){
                if(StringUtils.equals(tabName, HdtvConstant.tabName[i])){
        ...
        classAdapter.setOnItemClickListener(position -> {
            try{
                Intent intent = new Intent(this, HdtvVideoDetailActivity.class);
    ...
    private void setDataList() {
        try{
            ...
            new Thread(() -> {
                ...
                String searchValTemp = "";
                if(!StringUtils.isEmpty(searchword)){
                    searchValTemp = URLEncoder.encode(searchword);
                }
                String apiUrl = "";
                if(!StringUtils.isEmpty(searchValTemp)){
                    apiUrl = HdtvConstant.getReqDomain()+"/vod-search-pg-%s-wd-%s.html";
                    apiUrl = String.format(apiUrl,pageIndex,searchValTemp);
                }else{
                    if(StringUtils.isEmpty(categoryId)){
                        apiUrl = HdtvConstant.getReqDomain();
                        if(pageIndex > 1){
                            return;
                        }
                    }else{
                        apiUrl = HdtvConstant.getReqDomain()+"/vod-list-id-%s-pg-%s-order--by-%s-class--year-%s-letter--area-%s-lang-.html";
                        apiUrl = String.format(apiUrl,categoryId,pageIndex,orderBy,searchYear,searchRegionTemp);
                    }
                }
               ...
               /* if(StringUtils.isEmpty(searchword)){
                ...
                try{
                    ...
                    Elements tjs = document.select(".index-tj-l li a");
                    for(Element element: tjs) {
                        VideoInfoBean info =  new VideoInfoBean();
                        String link = element.attr("href");
                        String name = element.attr("title");
                        String img = element.select("img").attr("data-original");
                        String actor = element.select(".actor").text();
                        String keywords = element.select(".other").text();
                        info.setTitle(name);
                        info.setLink(HdtvConstant.getReqDomain()+link);
                        ...
                        if(!StringUtils.isEmpty(img)){
                            if(img.startsWith("http")){
                                ...
                                info.setCoverImage(HdtvConstant.getReqDomain()+img);
                            }
                        }
                    ...
                    Elements elements = document.select(".index-area li a");
