...
import com.vxiaokang.video.activity.video.kdy.bean.KdyConstant;
 ...
 * 列表
 */
public class KdyVideoListActivity extends AppCompatActivity implements View.OnClickListener{
    private String TAG = "KdyVideoListActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kdy_video_list);
    ...
    private void initSpinnerRegion(){
        String[] tabName = KdyConstant.regionName;
        //声明一个下拉列表的数组适配器
        ...
        regionSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchRegion = KdyConstant.regionId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerType(){
        String[] tabName = KdyConstant.typeName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        typeSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchType = KdyConstant.typeId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerLanguage(){
        String[] tabName = KdyConstant.languageName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        languageSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchLanguage = KdyConstant.languageId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerTab(){
        String[] tabName = KdyConstant.tabName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        tabSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                categoryId = KdyConstant.tabId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerYear(){
        String[] tabName = KdyConstant.yearName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        yearSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchYear = KdyConstant.yearId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerOrder(){
        String[] tabName = KdyConstant.orderName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        orderSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                orderBy = KdyConstant.orderId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initView() {
        ...
        if(!TextUtils.isEmpty(tabName)){
            ...
            for(int i = 0 ; i < KdyConstant.tabName.length ; i++){
                if(StringUtils.equals(tabName,KdyConstant.tabName[i])){
        ...
        try{
            ...
            new Thread(() -> {
                ...
                if(!StringUtils.isEmpty(searchRegion)){
                    searchRegionTemp = "/area/"+URLEncoder.encode(searchRegion);
                }
                String searchTypeTemp = ""; // class
                if(!StringUtils.isEmpty(searchType)){
                    searchTypeTemp = "/class/"+URLEncoder.encode(searchType);
                }
                String searchLanguageTemp = ""; //lang
                if(!StringUtils.isEmpty(searchLanguage)){
                    searchLanguageTemp = "/lang/"+URLEncoder.encode(searchLanguage);
                }
                String searchYearTemp = "";
                if(!StringUtils.isEmpty(searchYear)){
                    searchYearTemp = "/year/"+searchYear;
                }
                String searchOrderByTemp = "";
                if(!StringUtils.isEmpty(orderBy)){
                    searchOrderByTemp = "/by/"+orderBy;
                }
               ...
               // private String searchLanguage = "";
                ...
                if(StringUtils.isEmpty(searchword)){
                    // https://www.1080kdy.com/index.php/vod/show/area/%E5%A4%A7%E9%99%86/by/time/class/%E5%96%9C%E5%89%A7/id/6/lang/%E5%9B%BD%E8%AF%AD/letter/A/year/2018/page/2.html
                    apiUrl = KdyConstant.getReqDomain() +"/index.php/vod/show%s%s%s/id/%s%s%s/page/%s.html";
                    apiUrl = String.format(apiUrl,searchRegionTemp,searchOrderByTemp,searchTypeTemp,categoryId,searchLanguageTemp,searchYearTemp,pageIndex);
                    //apiUrl = KdyConstant.getReqDomain() +"/index.php/vod/type/id/%s/page/%s.html";
                   // apiUrl = String.format(apiUrl,categoryId,pageIndex);
                }else{
                    String searchwordTemp = URLEncoder.encode(searchword);
                    apiUrl = KdyConstant.getReqDomain() +"/index.php/vod/search/page/%s/wd/%s.html";
                    apiUrl = String.format(apiUrl,pageIndex,searchwordTemp);
                }
               // Log.d(TAG,"url = "+apiUrl);
                ...
                try{
                    ...
                    if(StringUtils.isEmpty(searchword)){
                        Elements elements = document.select("ul.myui-vodlist li");
                        for(Element element: elements) {
                            ...
                            info.setTitle(name);
                            info.setLink(KdyConstant.getReqDomain()+link);
                            ...
                            if(!StringUtils.isEmpty(img)){
                                if(img.startsWith("http")){
                                    ...
                                    info.setCoverImage(KdyConstant.getReqDomain()+img);
                                }
                            }
