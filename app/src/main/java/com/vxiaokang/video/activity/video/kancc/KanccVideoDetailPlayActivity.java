 ...
 * 课程详情列表
 */
public class KanccVideoDetailPlayActivity extends AppCompatActivity implements View.OnClickListener{
        ...
        try{
            if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position){
               ...
               // if(StringUtils.isEmpty(videoInfoBean.getLink())){
                KanccUtil.getUrl(videoInfoBean.getLink(), new KanccUtil.KanccListener() {
                        @Override
        ...
        try{
            new Thread(() -> {
                ...
                try{
                    ...
                    Elements tabs = document.select("ul.nav-tabs .item li");
    ...
    private void saveHistory(VideoInfoBean info,int position){
        try{
            ...
            if(null == daoSession){
                runOnUiThread(() -> {
                    Toast.makeText(KanccVideoDetailPlayActivity.this,"历史记录：daoSession is null",Toast.LENGTH_LONG).show();
                });
            }
            ...
            runOnUiThread(() -> {
                Toast.makeText(KanccVideoDetailPlayActivity.this,"历史记录出错："+e.getMessage(),Toast.LENGTH_LONG).show();
            });
            e.printStackTrace();
        }
    }
