...
import com.vxiaokang.video.activity.video.tube.TubeUtil;
import com.vxiaokang.video.activity.video.tube.bean.TubeConstant;
import com.vxiaokang.video.adapter.kdy.KdyVideoDetailAdapter;
import com.vxiaokang.video.adapter.kdy.KdyVideoDetailTabAdapter;
 ...
 * 课程详情列表
 */
public class KdyVideoDetailPlayActivity extends AppCompatActivity implements View.OnClickListener{
    private String TAG = "KdyVideoDetailPlayActivity";
    ...
    private KdyVideoDetailAdapter classAdapter;
    ...
    private KdyVideoDetailTabAdapter tabAdapter;
    ...
    private void initView() {
        ...
        tabAdapter = new KdyVideoDetailTabAdapter(this,tabList,currentPosition);
        ...
        classAdapter = new KdyVideoDetailAdapter(this,videoInfoList);
        ...
        try{
            if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position){
               ...
               // if(StringUtils.isEmpty(videoInfoBean.getLink())){
                TubeUtil.getUrl(videoInfoBean.getLink(), new TubeUtil.TubeListener() {
                        @Override
                        public void success(String path) {
                            Log.d(TAG,"KdyUtil.getUrl --"+path);
        ...
        try{
            new Thread(() -> {
                ...
                try{
                    ...
                    tabListTemp.add("列表");
                   /* Elements tabs = document.select("ul.pull-right ul.item li");
    ...
    private void saveHistory(VideoInfoBean info,int position){
        try{
            ...
            if(null == daoSession){
                runOnUiThread(() -> {
                    Toast.makeText(KdyVideoDetailPlayActivity.this,"历史记录：daoSession is null",Toast.LENGTH_LONG).show();
                });
            }
            ...
            runOnUiThread(() -> {
                Toast.makeText(KdyVideoDetailPlayActivity.this,"历史记录出错："+e.getMessage(),Toast.LENGTH_LONG).show();
            });
            e.printStackTrace();
        }
    }
