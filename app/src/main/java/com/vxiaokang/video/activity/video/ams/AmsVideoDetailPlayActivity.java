 ...
 * 课程详情列表
 */
public class AmsVideoDetailPlayActivity extends AppCompatActivity implements View.OnClickListener{
        ...
        try{
            if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position){
               ...
               // if(StringUtils.isEmpty(videoInfoBean.getLink())){
                AmsUtil.getUrl(videoInfoBean.getLink(), new AmsUtil.AmsListener() {
                        @Override
        ...
        try{
            new Thread(() -> {
                ...
                try{
                    ...
                    String m3u8Url =  document.select("span#fullVideoURL").text();
                    if(!StringUtils.isEmpty(m3u8Url)){
                       int index =  new Random().nextInt(AmsConstant.playUrl.size());
                        bodyUrl = AmsConstant.playUrl.get(index)+m3u8Url;
                    }
                    if(StringUtils.isEmpty(bodyUrl)){
                        Element linkBody = document.select("div.navigations").last().select("a").first();
                        bodyUrl = linkBody.attr("href");
                    }
                }catch (Exception e){
                    e.printStackTrace();
                }
