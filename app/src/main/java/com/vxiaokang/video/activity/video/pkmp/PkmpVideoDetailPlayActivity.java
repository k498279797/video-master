 ...
 * 课程详情列表
 */
public class PkmpVideoDetailPlayActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_pkmp_video_play_detail);
    ...
    private void initView() {
       ...
       // GridLayoutManager gridLayoutManager2 = new GridLayoutManager(this, 5);
        ...
        tabAdapter = new PkmpVideoDetailTabAdapter(this,tabList,currentPosition);
        ...
        try{
            if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position){
               ...
               // if(StringUtils.isEmpty(videoInfoBean.getLink())){
                    PkmpUtil.getUrl(videoInfoBean.getLink(), new PkmpUtil.PkmpListener() {
                        @Override
        ...
        try{
            new Thread(() -> {
                ...
                try{
                    ...
                    Elements tabs = document.select("div.box_con .type");
                    for(Element itElement : tabs) {
                        // System.out.print(itElement.text()+"  ");
                        tabListTemp.add(itElement.text());
                    }

                    Elements elements = document.select("ul.showplayul");
    ...
    private void saveHistory(VideoInfoBean info,int position){
        try{
            ...
            //https://www.pkmp4.com/py/66372-1-1.html
            ...
            int star = info.getLink().indexOf("?");
            int end = info.getLink().indexOf("-");
            ...
            if(null == daoSession){
                runOnUiThread(() -> {
                    Toast.makeText(PkmpVideoDetailPlayActivity.this,"历史记录：daoSession is null",Toast.LENGTH_LONG).show();
                });
            }
            ...
            runOnUiThread(() -> {
                Toast.makeText(PkmpVideoDetailPlayActivity.this,"历史记录出错："+e.getMessage(),Toast.LENGTH_LONG).show();
            });
            e.printStackTrace();
        }
    }
