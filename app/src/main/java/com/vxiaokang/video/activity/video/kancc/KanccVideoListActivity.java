 ...
 * 列表
 */
public class KanccVideoListActivity extends AppCompatActivity implements View.OnClickListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kancc_video_list);
    ...
    private void initSpinnerRegion(){
        String[] tabName = KanccConstant.regionName;
        //声明一个下拉列表的数组适配器
        ...
        regionSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchRegion = KanccConstant.regionId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerType(){
        String[] tabName = KanccConstant.typeName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        typeSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchType = KanccConstant.typeId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerLanguage(){
        String[] tabName = KanccConstant.languageName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        languageSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchLanguage = KanccConstant.languageId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerTab(){
        String[] tabName = KanccConstant.tabName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        tabSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                String categoryIdTemp = KanccConstant.tabId[position]+"";
                if(!StringUtils.equals(categoryIdTemp,categoryId)){
    ...
    private void initSpinnerYear(){
        String[] tabName = KanccConstant.yearName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        yearSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                searchYear = KanccConstant.yearId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initSpinnerOrder(){
        String[] tabName = KanccConstant.orderName;
        //声明一个下拉列表的数组适配器
        ArrayAdapter<String> starAdapter = new ArrayAdapter<String>(this,R.layout.item_select,tabName);
        //设置数组适配器的布局样式
        starAdapter.setDropDownViewResource(R.layout.item_dropdown);
        ...
        orderSp.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
                orderBy = KanccConstant.orderId[position]+"";
                if(initFlag){
                    setDataList();
                }
            }
    ...
    private void initView() {
        ...
        if(!TextUtils.isEmpty(tabName)){
            ...
            for(int i = 0; i < KanccConstant.tabName.length ; i++){
                if(StringUtils.equals(tabName, KanccConstant.tabName[i])){
        ...
        classAdapter.setOnItemClickListener(position -> {
            try{
                Intent intent = new Intent(this, KanccVideoDetailActivity.class);
        ...
        try{
            ...
            new Thread(() -> {
                ...
                if(StringUtils.isEmpty(categoryId)){
                    apiUrl = KanccConstant.getReqDomain();
                }else{
                    apiUrl = KanccConstant.getReqDomain()+"/show/%s--%s------%s---.html";
                    apiUrl = String.format(apiUrl,categoryId,orderBy,pageIndex);
                }
                if(pageIndex >1 && StringUtils.isEmpty(categoryId)){
                    return;
                }
                ...
                try{
                    ...
                    for(Element element: elements) {
                        ...
                        String img = element.select("a").first().attr("data-original");
                        String name = element.select(".title").text();
                        String actor = element.select(".text").text();
                        String keywords = element.select(".pic-text").text();
                        ...
                        System.out.println();*/
                        info.setTitle(name);
                        info.setLink(KanccConstant.getReqDomain()+link);
                        ...
                        if(!StringUtils.isEmpty(img)){
                            if(img.startsWith("http")){
                                ...
                                info.setCoverImage(KanccConstant.getReqDomain()+img);
                            }
                        }
