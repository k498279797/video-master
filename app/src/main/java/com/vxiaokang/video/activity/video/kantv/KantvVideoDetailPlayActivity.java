 ...
 * 课程详情列表
 */
public class KantvVideoDetailPlayActivity extends AppCompatActivity implements View.OnClickListener{
    private String TAG = "KanccVideoDetailPlayActivity";
        ...
        try{
            if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position){
               ...
               // if(StringUtils.isEmpty(videoInfoBean.getLink())){
                KantvUtil.getUrl(videoInfoBean.getLink(), new KantvUtil.KantvListener() {
                        @Override
                    ...
                    for(Element itElement : elements) {
                        ...
                        for(Element info : aElements) {
                           ...
                           // infoBean.setCoverImage(img);
                            infoBean.setLink(KantvConstant.getReqDomain()+info.attr("href"));
    ...
    private void saveHistory(VideoInfoBean info,int position){
        try{
            ...
            if(null == daoSession){
                runOnUiThread(() -> {
                    Toast.makeText(KantvVideoDetailPlayActivity.this,"历史记录：daoSession is null",Toast.LENGTH_LONG).show();
                });
            }
            ...
            runOnUiThread(() -> {
                Toast.makeText(KantvVideoDetailPlayActivity.this,"历史记录出错："+e.getMessage(),Toast.LENGTH_LONG).show();
            });
            e.printStackTrace();
        }
    }
