...
import com.vxiaokang.video.adapter.YinshiVideoDetailAdapter;
import com.vxiaokang.video.adapter.YinshiVideoDetailTabAdapter;
 ...
 * 课程详情列表
 */
public class YinshiVideoDetailPlayActivity extends AppCompatActivity implements View.OnClickListener{
    private String TAG = "YinshiVideoDetailPlayActivity";
    ...
    private YinshiVideoDetailAdapter classAdapter;
    ...
    private YinshiVideoDetailTabAdapter tabAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_yinshi_video_play_detail);
    ...
    private void initView() {
        ...
        tabAdapter = new YinshiVideoDetailTabAdapter(this,tabList,currentPosition);
        ...
        classAdapter = new YinshiVideoDetailAdapter(this,videoInfoList);
        ...
        try{
            if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position){
               ...
               // if(StringUtils.isEmpty(videoInfoBean.getLink())){
                YinshiUtil.getUrl(videoInfoBean.getLink(), new YinshiUtil.YinshiListener() {
                        @Override
        ...
        try{
            new Thread(() -> {
                ...
                try{
                    ...
                    Elements tabs = document.select("ul.play-tab li");
                 ...
                 //   Elements elements = document.select("ul.stui-content__playlist");
                    Elements elements = document.select(".play-item>ul");
                    for(Element itElement : elements) {
                        ...
                        for(Element info : aElements) {
                           ...
                           // infoBean.setCoverImage(img);
                            infoBean.setLink(YinshiConstant.getReqDomain()+info.attr("href"));
    ...
    private void saveHistory(VideoInfoBean info,int position){
        try{
            ...
            if(null == daoSession){
                runOnUiThread(() -> {
                    Toast.makeText(YinshiVideoDetailPlayActivity.this,"历史记录：daoSession is null",Toast.LENGTH_LONG).show();
                });
            }
            ...
            runOnUiThread(() -> {
                Toast.makeText(YinshiVideoDetailPlayActivity.this,"历史记录出错："+e.getMessage(),Toast.LENGTH_LONG).show();
            });
            e.printStackTrace();
        }
    }
