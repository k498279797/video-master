 ...
 * 课程详情列表
 */
public class ShipinVideoDetailPlayActivity extends AppCompatActivity implements View.OnClickListener{
    private String TAG = "ShipinVideoDetailPlayActivity";
    ...
    private ShipinVideoDetailAdapter classAdapter;
    ...
    private ShipinVideoDetailTabAdapter tabAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_shipin_video_play_detail);
    ...
    private void initView() {
      ...
      //  GridLayoutManager gridLayoutManager2 = new GridLayoutManager(this, 5);
        ...
        tabAdapter = new ShipinVideoDetailTabAdapter(this,tabList,currentPosition);
        ...
        classAdapter = new ShipinVideoDetailAdapter(this,videoInfoList);
        ...
        try{
            if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position){
               ...
               // if(StringUtils.isEmpty(videoInfoBean.getLink())){
                    ShipinUtil.getUrl(videoInfoBean.getLink(), new ShipinUtil.ShipinListener() {
                        @Override
    ...
    public void playVideo(VideoInfoBean videoInfoBean,String url,int position){
        try{
            new Handler().postDelayed(() -> saveHistory(videoInfoBean,position), 20);
            ...
            //if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position){
             //   VideoInfoBean videoInfoBean =  videoInfoList.get(position);
                ...
                dkPlayer.setUrl(url);
                StandardVideoController controller = new StandardVideoController(this);
                controller.addDefaultControlComponent(currentTitle, false);
                dkPlayer.setVideoController(controller);
        ...
        try{
            new Thread(() -> {
                ...
                try{
                    ...
                    Elements tabs = document.select("dt.tabt3 span");
                    for(Element itElement : tabs) {
                        // System.out.print(itElement.text()+"  ");
                        tabListTemp.add(itElement.text());
                    }

                    Elements elements = document.select("div.ulNumList");
                    for(Element itElement : elements) {
                        ...
                        for(Element info : aElements) {
                           ...
                           // infoBean.setCoverImage(img);
                            infoBean.setLink(ShipinConstant.getReqDomain()+info.attr("href"));
                    ...
                    String one = "now=base64decode(";
                    if(body.indexOf("now=base64decode(") != -1) {
                        body = body.substring(body.indexOf("now=base64decode(")+one.length()+1);
                        body = body.substring(0, body.indexOf(")")-1);
                        bodyUrl = new String(Base64.decodeBase64(body));
    ...
    private void saveHistory(VideoInfoBean info,int position){
        try{
          ...
          //  Log.d(TAG,"保存生成记录..."+info.getLink());
            ...
            // https://www.88ysw.com/play/?283569-16-1.html
            if(null == historyRecord && !StringUtils.isEmpty(info.getLink()) && info.getLink().lastIndexOf("?") != -1){
                videoKey = info.getLink().substring(info.getLink().lastIndexOf("?")+1);
                videoKey = videoKey.replace(".html", "");
                if(!StringUtils.isEmpty(videoKey) ){
                    if(videoKey.contains("-")){
                        videoKey = videoKey.substring(0,videoKey.indexOf("-"));
                    }
            ...
            if(null == daoSession){
                runOnUiThread(() -> {
                    Toast.makeText(ShipinVideoDetailPlayActivity.this,"历史记录：daoSession is null",Toast.LENGTH_LONG).show();
                });
            }
            ...
            runOnUiThread(() -> {
                Toast.makeText(ShipinVideoDetailPlayActivity.this,"历史记录出错："+e.getMessage(),Toast.LENGTH_LONG).show();
            });
            e.printStackTrace();
        }
    }
