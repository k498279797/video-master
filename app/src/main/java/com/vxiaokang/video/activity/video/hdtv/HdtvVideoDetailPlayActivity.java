...
import com.vxiaokang.video.activity.video.hdtv.bean.HdtvConstant;
...
import com.vxiaokang.video.util.commons.Escape;
 ...
 * 课程详情列表
 */
public class HdtvVideoDetailPlayActivity extends AppCompatActivity implements View.OnClickListener{
    private String TAG = "HdtvVideoDetailPlayActivity";
    ...
    private List<List<String>> urlAllList = new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_kantv_video_play_detail);
    ...
    private void initView() {
       ...
       // GridLayoutManager gridLayoutManager2 = new GridLayoutManager(this, 5);
        ...
        tabAdapter = new KantvVideoDetailTabAdapter(this,tabList,currentPosition);
        ...
        tabAdapter.setOnItemClickListener(position -> {
            try{
                ...
                urlInfoList = urlAllList.get(position);
    ...
    public void updateVideoInfo(int position){
        try{
            if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position  && null != urlInfoList && urlInfoList.size()-1>= position){
                VideoInfoBean videoInfoBean =  videoInfoList.get(position);
                String videoUrl =  urlInfoList.get(position);
               ...
               // if(StringUtils.isEmpty(videoInfoBean.getLink())){
                HdtvUtil.getUrl(videoUrl, new HdtvUtil.HdtvListener() {
                        @Override
                        public void success(String path) {
                            if(StringUtils.isEmpty(path) || path.endsWith(".html")){
                                Looper.prepare();
                                Toast.makeText(App.getContext(), "出错地址", Toast.LENGTH_SHORT).show();
                                Looper.loop();
                            }else{
            ...
            //if(position>=0 && null != videoInfoList && videoInfoList.size()-1>= position){
             //   VideoInfoBean videoInfoBean =  videoInfoList.get(position);
               try{
                ...
                currentTitle = (StringUtils.isEmpty(paramsBean.getSourceItemName()) ? ""+videoInfoBean.getTitle() :paramsBean.getSourceItemName())+videoInfoBean.getTitle();
                currVideoTitle.setText(currentTitle);
    ...
    private void setDataList(String apiUrl) {
        try{
            new Thread(() -> {
                ...
                List<List<String>> urlAllTemp = new ArrayList<>();
                ...
                try{
                    ...
                    Elements tabs =  document.select("div.playfrom li");
                    for(Element itElement : tabs) {
                        // System.out.print(itElement.text()+"  ");
                        tabListTemp.add(itElement.text());
                    }

                    Elements elements = document.select(".videourl>ul");
                    for(Element itElement : elements) {
                        ...
                        for(Element info : aElements) {
                            ...
                            if(!StringUtils.isEmpty(paramsBean.getCoverImage())){
                                infoBean.setCoverImage(paramsBean.getCoverImage());
                            }
                            infoBean.setLink(HdtvConstant.getReqDomain()+info.attr("href"));
                    ...
                    String one = "mac_url=unescape('";
                    String mac_url = "";
                    String current = "";
                    if(body.indexOf(one)!= -1) {
                        mac_url = body.substring(body.indexOf(one)+one.length());
                        mac_url = mac_url.substring(0, mac_url.indexOf("');"));
                        mac_url = mac_url.trim();
                        mac_url = Escape.unescape(mac_url);
                        // System.out.println(mac_url);
                        String mac_urla[] = mac_url.split("\\$\\$\\$");
                        for(String line: mac_urla) {
                            String mac_urlb[] = line.split("#");
                            List<String> urlTemp = new ArrayList<>();
                            for(String bString : mac_urlb) {
                                String url = bString.split("\\$")[1];
                                urlTemp.add(url);
                                if(StringUtils.isEmpty(current)){
                                    current = url;
                                    bodyUrl = current;
                                }
                                //System.out.println(bString.split("\\$")[0]+"  "+bString.split("\\$")[1]);
                            }
                            urlAllTemp.add(urlTemp);
                        }
                    }
                }catch (Exception e){
                    e.printStackTrace();
                ...
                runOnUiThread(()->{
                    if(dataListAllTemp.size() > 0){
                        ...
                        urlAllList = urlAllTemp;
                        videoInfoList = dataListAllTemp.get(0);
                        urlInfoList = urlAllList.get(0);
                        ...
                        // updateUI(itemTemp);
                        HdtvUtil.getUrl(finalBodyUrl, new HdtvUtil.HdtvListener() {
                            @Override
                            public void success(String path) {
                                currentPlayUrl = path;
                                if(StringUtils.isEmpty(path) || path.endsWith(".html")){
    ...
    private String getVideoKeyFromLink(String link){
        String videoKey = link;
        String  videoKeyTemp = link.substring(link.lastIndexOf("/")+1);
        if(!StringUtils.isEmpty(videoKeyTemp)){
            String one = "vod-play-id-";
            if(videoKeyTemp.contains("-") && videoKeyTemp.contains(one)){
                videoKeyTemp = videoKeyTemp.substring(videoKeyTemp.indexOf(one)+one.length());
    ...
    private void saveHistory(VideoInfoBean info,int position){
        try{
           ...
           // http://88hd.com/vod-play-id-213801-src-1-num-2.html
            ...
            if(null == daoSession){
                runOnUiThread(() -> {
                    Toast.makeText(HdtvVideoDetailPlayActivity.this,"历史记录：daoSession is null",Toast.LENGTH_LONG).show();
                });
            }
            ...
            runOnUiThread(() -> {
                Toast.makeText(HdtvVideoDetailPlayActivity.this,"历史记录出错："+e.getMessage(),Toast.LENGTH_LONG).show();
            });
            e.printStackTrace();
        }
    }
